document.addEventListener("DOMContentLoaded", () => {
  ...
  function showImportReview(preview, payload) {
    const courseDetails = preview.courses.map((plan) => {
      ...
      return `<div class="ba-import-review-course"><strong>${escapeHtml(course?.name || "Course")}</strong><span>Worksheet: ${escapeHtml(map.sheet || "-")}</span><span>Student Mapping: ${escapeHtml(map.username || "-")}</span><span>${plan.rows} students found</span>${notFound ? `<span>${notFound} students not found</span>` : ""}${missing ? `<span>${missing} students ${escapeHtml(plan.usernameheader || "Username")} missing</span>` : ""}</div>`;
    }).join("");
